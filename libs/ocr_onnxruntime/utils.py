...
class OrtInferSession:
    def __init__(self, config):
        ...
        if config["use_cuda"] and get_device() == "GPU" and cuda_ep in get_available_providers():
            execution_providers.append((cuda_ep, cuda_provider_options))
        else:
            execution_providers.append((cpu_ep, cpu_provider_options))
            sess_opt.enable_cpu_mem_arena = False
            sess_opt.graph_optimization_level = GraphOptimizationLevel.ORT_ENABLE_ALL
            sess_opt.intra_op_num_threads = config["num_threads"]
            sess_opt.execution_mode = ExecutionMode(config["execution_mode"])
            sess_opt.inter_op_num_threads = config["num_threads"]

        self._verify_model(config["model_path"])
        self.session = InferenceSession(config["model_path"], sess_options=sess_opt, providers=execution_providers)

        if config["use_cuda"] and cuda_ep not in self.session.get_providers():
            warnings.warn(
                f"{cuda_ep} is not avaiable for current env, the inference part is automatically shifted to be "
...
class UpdateParameters:
    ...
    def update_params(
        ...
        if not model_path:
            filter_dict["model_path"] = str(project_root / config["model_path"])
    ...
    def get_font_path(font_path: Optional[Union[str, Path]] = None) -> str:
        if font_path is None or not Path(font_path).exists():
            raise FileNotFoundError(
                ...
                f"You could download the file in the "
                f"https://drive.google.com/file/d/1evWVX38EFNwTq_n5gTFgnlv8tdaNcyIA/view?usp=sharing "
            )
